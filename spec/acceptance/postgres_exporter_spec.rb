...
describe 'prometheus postgres exporter' do
  ...
  describe 'postgres_exporter update from 0.5.1 to 0.8.0' do
    it 'is idempotent' do
      pp = "class{'prometheus::postgres_exporter': postgres_auth_method => 'custom', group => 'postgres', user => 'postgres', data_source_custom => {'DATA_SOURCE_NAME' => 'user=postgres host=/var/run/postgresql/ sslmode=disable',}, version => '0.5.1'}"
      # Run it twice and test for idempotency
      apply_manifest(pp, catch_failures: true)
    ...
    it 'is idempotent' do
      pp = "class{'prometheus::postgres_exporter': postgres_auth_method => 'custom', group => 'postgres', user => 'postgres', data_source_custom => {'DATA_SOURCE_NAME' => 'user=postgres host=/var/run/postgresql/ sslmode=disable',}, version => '0.8.0'}"
      # Run it twice and test for idempotency
      apply_manifest(pp, catch_failures: true)
      apply_manifest(pp, catch_changes: true)
    end
